...
public class BookMessageDao implements IBookMessageDao{
    ...
    @Override
    public Book queryBookById(int bookId) throws SQLException, NoSuchFieldException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        String sql = "SELECT * FROM user_book  WHERE book_id = ?;";
        List<Book> books = connection.executeQuery(Book.class, sql, bookId);
        return books.get(0);
    }
    @Override
    ...
    public int updateBook(Book book) throws SQLException {
        String sql = "UPDATE user_book SET book_name = ?,user_student_number = ?,book_status = ?,book_price = ? WHERE book_id = ?;";
        int flag = connection.executeUpdate(sql, book.getBookName(), book.getUserStudentNumber(), book.getBookStatus(), book.getBookPrice(), book.getBookId());
        if(flag>0){
            return 1;
        }
        return -1;
